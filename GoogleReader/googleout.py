...
def render_header():
    ...
    <link rel='stylesheet' type='text/css' href='googleout.css'>
    </head><body>
...
<a href='DougStarred001.html'>Starred</a>
 ...
 - <a href='DougLiked001.html'>Liked</a>
</div>
    <div class='container'>
    '''
    # <style>div {border: 2px red solid;} span {border: 1px blue solid;}</style>
...
def render_item(readerItem=None, ItemNumber=0):
    ...
    ItemString += "<span class='itemorigin clearfix span8' id = 'item" + str(ItemNumber) + "'>"  
    ...
    if "title" in readerItem:
        ItemString += "<span class='itemtitle em'>" + UrlString + readerItem["title"] + "</a></span>"

    if "content" in readerItem:
        ItemString += "\n<div class='itemcontent clearfix span5'>" + readerItem["content"]["content"]

    if "content" not in readerItem:
        if "summary" in readerItem:
            ItemString += "\n<div class='itemcontent clearfix'>" + readerItem["summary"]["content"]
        else:
...
def CloseCurrentPage(pagenumber, Output, outputfilename, Pages):
    ...
    filename = "jody/" + outputfilename + str(pagenumber).zfill(3) + ".html"
...
txt = open('/Users/doug.churchman/Documents/src/jody/notes.json')
sharedby = json.loads(txt.read().decode("utf-8"))
build_file_items(sharedby, "JodyNotes")
...
# build_file_items(liked, "DougLiked")
...
# txt = open('/Users/doug.churchman/Documents/src/jody/shared-by-followers.json')
# sharedby = json.loads(txt.read().decode("utf-8"))
# build_file_items(sharedby, "JodyFollowers")

# txt = open('/Users/doug.churchman/Documents/src/jody/shared.json')
# shared = json.loads(txt.read().decode("utf-8"))
# build_file_items(shared, "JodyShared")

# txt = open('/Users/doug.churchman/Documents/src/jody/starred.json')
# shared = json.loads(txt.read().decode("utf-8"))
# build_file_items(shared, "JodyStarred")

# txt = open('/Users/doug.churchman/Documents/src/jody/liked.json')
# liked = json.loads(txt.read().decode("utf-8"))
# build_file_items(liked, "JodyLiked")